...
make_olm_bundle() {
  #
  local repo_org="quay.io/rhobs"
  (
    cd rhobs/olm
    sed \
      -e "s|<IMG_PROMETHEUS_OPERATOR>|$repo_org/obo-prometheus-operator:v$RHOBS_VERSION|g" \
      -e "s|<IMG_PROMETHEUS_CONFIG_RELOADER>|$repo_org/obo-prometheus-config-reloader:v$RHOBS_VERSION|g" \
      -i manifests/operator/kustomization.yaml
    make tools
    rm -rf bundle && make bundle
  )
}
